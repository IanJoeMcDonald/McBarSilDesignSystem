//
//  MBSDSSecureField.swift
//  McBarSilDesignSystem
//
//  Created by Ian McDonald on 01/03/25.
//

import SwiftUI

public struct MBSDSSecureField: View {

    // MAR: Enum
    enum FieldToFocus {
        case textField
        case secureField
    }

    // MARK: Initializers
    public init(
        _ title: LocalizedStringResource,
        text: Binding<String>,
        message: Binding<MBSDSTextField.Message?>,
        isInverse: Bool = false,
        isSecure: Bool = true,
        style: MBSDSBorder.Style = .primary,
        backgroundColor: MBSDSColor = .background(.base)
    ) {
        self.title = title
        self.isInverse = isInverse
        self.style = style
        self.backgroundColor = backgroundColor.color
        self._text = text
        self._message = message
        self.isSecure = isSecure
        self.isEmpty = text.wrappedValue.isEmpty

    }

    // MARK: Environment
    @Environment(\.isEnabled) private var isEnabled

    // MARK: Binding
    @Binding private var text: String
    @Binding private var message: MBSDSTextField.Message?

    // MARK: State
    @State private var isSecure: Bool
    @State private var isEmpty: Bool
    @State private var height: Double = .zero
    @FocusState private var isFieldFocus: FieldToFocus?

    // MARK: Variables
    public var title: LocalizedStringResource
    public var isInverse: Bool
    public var style: MBSDSBorder.Style
    private var backgroundColor: Color

    // MARK: Body
    public var body: some View {
        VStack {
            ZStack(alignment: .leading) {
                HStack {
                    Group {
                        if isSecure {
                            SecureField("", text: $text)
                                .onChange(
                                    of: text,
                                    perform: { _ in
                                        isEmpty = text.isEmpty
                                    }
                                )
                                .focused($isFieldFocus, equals: .textField)
                                .padding(.leading)
                                .padding(.vertical)
                                .saveHeight(in: $height)
                        } else {
                            TextField("", text: $text)
                                .onChange(
                                    of: text,
                                    perform: { _ in
                                        isEmpty = text.isEmpty
                                    }
                                )
                                .focused($isFieldFocus, equals: .secureField)
                                .padding(.leading)
                                .padding(.vertical)
                                .saveHeight(in: $height)
                        }
                    }
                    .disableAutocorrection(true)
                    .autocapitalization(.none)
                    Button { isSecure.toggle() } label: { Image(systemName: self.isSecure ? "eye" : "eye.slash")
                            .accentColor(generateButtonColor())
                    }
                    .padding(.trailing)
                }
                .onChange(of: isSecure) { result in
                    isFieldFocus = isSecure ? .secureField : .textField
                }
                .background(
                    MBSDSTextField.BorderWithTitle(
                        title: title,
                        isInverse: isInverse,
                        style: style,
                        backgroundColor: backgroundColor,
                        isEmpty: $isEmpty,
                        height: $height)
                )
            }
            if let message {
                let messageText = switch message {
                case let .error(error): error
                case let .info(info): info
                case let .success(success): success
                case let .warning(warning): warning
                }

                let icon = switch message {
                case .error:
                    Image(systemName: "xmark.shield.fill")
                        .symbolRenderingMode(.palette)
                        .foregroundStyle(MBSDSColor.neutral(.base).color,MBSDSColor.status(.error).color)
                case .info:
                    Image(systemName: "exclamationmark.triangle.fill")
                        .symbolRenderingMode(.palette)
                        .foregroundStyle(MBSDSColor.neutral(.base).color, MBSDSColor.status(.informative).color)
                case .success:
                    Image(systemName: "checkmark.circle.fill")
                        .symbolRenderingMode(.palette)
                        .foregroundStyle(MBSDSColor.neutral(.base).color, MBSDSColor.status(.success).color)
                case .warning:
                    Image(systemName: "exclamationmark.shield.fill")
                        .symbolRenderingMode(.palette)
                        .foregroundStyle(MBSDSColor.neutral(.base).color, MBSDSColor.status(.warning).color)
                }
                HStack {
                    icon
                        .padding(.leading, MBSDSSpacing.sm.rawValue)
                    MBSDSLabel(text: messageText, font: .heading(.md), onColor: .emphasis(.medium))
                    Spacer()
                }
            } else {
                MBSDSLabel(text: " ", font: .heading(.md), onColor: .emphasis(.disabled))
            }
        }
    }

    // MARK: Private Custom Methods
    private func generateButtonColor() -> Color {
        switch style {
        case .primary: generatePrimaryButtonColor()
        case .neutral: generateNeutralButtonColor()
        }
    }

    private func generatePrimaryButtonColor() -> Color {
        if isEnabled {
            if isInverse {
                MBSDSColor.action(.secondary(.enabled)).inverseColor
            } else {
                MBSDSColor.action(.secondary(.enabled)).color
            }
        } else {
            if isInverse {
                MBSDSColor.action(.secondary(.enabled)).color
            } else {
                MBSDSColor.action(.secondary(.enabled)).inverseColor
            }
        }
    }

    private func generateNeutralButtonColor() -> Color {
        if isEnabled {
            if isInverse {
                MBSDSColor.action(.neutral(.enabled)).color
            } else {
                MBSDSColor.action(.neutral(.enabled)).inverseColor
            }
        } else {
            if isInverse {
                MBSDSColor.action(.disabled).inverseColor
            } else {
                MBSDSColor.action(.disabled).color
            }
        }
    }
}

// MARK: Preview
#Preview {
    struct Preview: View {
        @State var myString = ""
        @State var message1: MBSDSTextField.Message? = nil
        @State var message2: MBSDSTextField.Message? = .error("Error")
        @State var message3: MBSDSTextField.Message? = .info("Info")
        @State var message4: MBSDSTextField.Message? = .success("Success")
        @State var message5: MBSDSTextField.Message? = .warning("Warning")
        var body: some View {
            VStack(spacing: MBSDSSpacing.sm.rawValue) {
                MBSDSSecureField("Senha", text: $myString, message: $message1)
                //MBSDSSecureField("Title", text: $myString, message: $message2)
                //MBSDSSecureField("Title", text: $myString, message: $message3)
                //MBSDSSecureField("Title", text: $myString, message: $message4)
                //MBSDSSecureField("Title", text: $myString, message: $message5)
            }
        }
    }

    return Preview()
}
